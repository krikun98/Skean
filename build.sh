...
do
	...
	do
		...
		${container_cmd} yaqwsx/kikit:nightly pcbdraw --style builtin:set-blue-hasl.json "$file" images/"$name"_"$short_option".png >> /dev/null
		${container_cmd} yaqwsx/kikit:nightly pcbdraw --style builtin:set-blue-hasl.json --back "$file" images/"$name"_"$short_option"_back.png >> /dev/null
	done
done
